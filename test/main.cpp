...
int main(int argc, char **argv) {
  ...
  // Init data
  Matrix d_dense_mat_cpu =  rand({dim, k}, Device(kCPU), DataType(kFloat), -1.0, 1.0);
  Matrix d_dense_mat_cuda = d_dense_mat_cpu.to(kCUDA);
  ...
  // d_result_csr_spmm_bench.to(kCPU).print();
#endif
  ...
  // ExpectEqualDenseWithError(d_result_coo_spmm, d_result_csr_spmm_bench);
  cudaDeviceSynchronize();
#endif
  ...
  LBSpMM(d_coo_mat, d_dense_mat_cuda, d_result_coo_spmm, spmm_hp_time);
  info.spmm_hp_time_ = spmm_hp_time;
  // d_result_coo_spmm.to(kCPU).print();
  #ifdef CHECK_RES
    ExpectEqualDenseWithError(d_result_coo_spmm, d_result_csr_spmm_bench, "div");
  #endif
  cudaDeviceSynchronize();
#endif
  write_info_to_file("./full_graph_res.csv", info);
  return 1;
}