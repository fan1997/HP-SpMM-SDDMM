...
# Version history
        ...
        * Version 1.2.2 - 2016-03-29 - Fix issues with set_fatal_handler
throwing an exception
        * Version 1.2.3 - 2016-05-16 - Log current working directory in
loguru::init().
        * Version 1.2.4 - 2016-05-18 - Custom replacement for -v in
loguru::init() by bjoernpollex
        * Version 1.2.5 - 2016-05-18 - Add ability to print ERROR_CONTEXT of
parent thread.
        * Version 1.2.6 - 2016-05-19 - Bug fix regarding VLOG verbosity argument
lacking ().
        ...
        * Version 1.3.0 - 2016-07-20 - Fix issues with callback flush/close not
being called.
        * Version 1.3.1 - 2016-07-20 - Add LOGURU_UNSAFE_SIGNAL_HANDLER to
toggle stacktrace on signals.
        * Version 1.3.2 - 2016-07-20 - Add loguru::arguments()
        * Version 1.4.0 - 2016-09-15 - Semantic versioning + add
loguru::create_directories
        * Version 1.4.1 - 2016-09-29 - Customize formating with
LOGURU_FILENAME_WIDTH
        * Version 1.5.0 - 2016-12-22 - LOGURU_USE_FMTLIB by kolis and
LOGURU_WITH_FILEABS by scinart
        * Version 1.5.1 - 2017-08-08 - Terminal colors on Windows 10 thanks to
looki
        * Version 1.6.0 - 2018-01-03 - Add LOGURU_RTTI and LOGURU_STACKTRACES
settings
        * Version 1.7.0 - 2018-01-03 - Add ability to turn off the preamble with
loguru::g_preamble
        * Version 1.7.1 - 2018-04-05 - Add function get_fatal_handler
        * Version 1.7.2 - 2018-04-22 - Fix a bug where large file names could
cause stack corruption (thanks @ccamporesi)
        * Version 1.8.0 - 2018-04-23 - Shorten long file names to keep preamble
fixed width
        * Version 1.9.0 - 2018-09-22 - Adjust terminal colors, add
LOGURU_VERBOSE_SCOPE_ENDINGS, add LOGURU_SCOPE_TIME_PRECISION, add named log
levels
        * Version 2.0.0 - 2018-09-22 - Split loguru.hpp into loguru.hpp and
loguru.cpp
        * Version 2.1.0 - 2019-09-23 - Update fmtlib + add option to
loguru::init to NOT set main thread name.
...
# Usage
        For details, please see the official documentation at
emilk.github.io/loguru
...
// See [NOTE ABOUT LOG] in project/CMakeLists.txt
#ifndef STARML_USE_THROW_EXCEPTION
#ifdef NDEBUG
#define STARML_USE_THROW_EXCEPTION 1
#else
#define STARML_USE_THROW_EXCEPTION 0
#endif
#endif
...
enum NamedVerbosity : Verbosity {
  ...
  // Don not use higher verbosity levels, as that will make grepping log files
  // harder.
  Verbosity_MAX = +9,
};

struct Message {
  // You would generally print a Message by just concating the buffers without
  // spacing. Optionally, ignore preamble and indentation.
...
LOGURU_EXPORT extern unsigned
    g_flush_interval_ms;  // 0 (unbuffered) by default.
...
/* Returns the thread name for this thread.
   On OSX this will return the system thread name (settable from both within and
   without Loguru). On other systems it will return whatever you set in
   set_thread_name(); If no thread name is set, this will return a hexadecimal
   thread id. length should be the number of bytes available in the buffer. 17
   is a good number for length. right_align_hext_id means any hexadecimal thread
   id will be written to the end of buffer.
*/
LOGURU_EXPORT
void get_thread_name(char* buffer, unsigned long long length,
                     bool right_align_hext_id);
...
#define DEFINE_CHECK_OP_IMPL(name, op)                                         \
  ...
  inline std::string* name(const char* expr, const T1& v1, const char* op_str, \
                           const T2& v2) {                                     \
  ...
  inline std::string* name(const char* expr, int v1, const char* op_str,       \
                           int v2) {                                           \
...
#define STARML_LOG_PREFIX 1
#if STARML_LOG_PREFIX

// NOTE: LOG() is only for developers, its outputs are invisible in release
// version. Use CHECK_X() if you want to tell user something when error
// occurred.
#define STARML_VLOG_IF(verbosity, cond) VLOG_IF_S(verbosity, cond)
#define STARML_LOG_IF(verbosity_name, cond) LOG_IF_S(verbosity_name, cond)
#define STARML_VLOG(verbosity) VLOG_S(verbosity)
#define STARML_LOG(verbosity_name) LOG_S(verbosity_name)
#define STARML_DLOG(verbosity_name) DLOG_S(verbosity_name)
#define STARML_DVLOG(verbosity) DVLOG_S(verbosity)
#define STARML_DLOG_IF(verbosity_name, cond) DLOG_IF_S(verbosity_name, cond)
#define STARML_DVLOG_IF(verbosity, cond) DVLOG_IF_S(verbosity, cond)

#define STARML_CHECK(cond) CHECK_S(!!(cond))
#define STARML_CHECK_NOTNULL(x) CHECK_NOTNULL_S(x)
#define STARML_CHECK_EQ(expr1, expr2) CHECK_EQ_S(expr1, expr2)
#define STARML_CHECK_NE(expr1, expr2) CHECK_NE_S(expr1, expr2)
#define STARML_CHECK_LT(expr1, expr2) CHECK_LT_S(expr1, expr2)
#define STARML_CHECK_LE(expr1, expr2) CHECK_LE_S(expr1, expr2)
#define STARML_CHECK_GT(expr1, expr2) CHECK_GT_S(expr1, expr2)
#define STARML_CHECK_GE(expr1, expr2) CHECK_GE_S(expr1, expr2)
#define STARML_DCHECK(cond) DCHECK_S(cond)
#define STARML_DCHECK_NOTNULL(x) DCHECK_NOTNULL_S(x)
#define STARML_DCHECK_EQ(a, b) DCHECK_EQ_S(a, b)
#define STARML_DCHECK_NE(a, b) DCHECK_NE_S(a, b)
#define STARML_DCHECK_LT(a, b) DCHECK_LT_S(a, b)
#define STARML_DCHECK_LE(a, b) DCHECK_LE_S(a, b)
#define STARML_DCHECK_GT(a, b) DCHECK_GT_S(a, b)
#define STARML_DCHECK_GE(a, b) DCHECK_GE_S(a, b)

#define STARML_CHECK_TYPE_MATCH(cond) \
  CHECK_S(!!(cond)) << " {STARML::TypeMismatch} "
#define STARML_CHECK_DIMS_MATCH(cond) \
  CHECK_S(!!(cond)) << " {STARML::kDimensionsMismatch} "
#define STARML_CHECK_ARGUMENT(cond) \
  CHECK_S(!!(cond)) << " {STARML::kInvalidArgument} "
#define STARML_CHECK_ALLOC(cond) CHECK_S(!!(cond)) << " {STARML::kAllocFailed} "
#define STARML_CHECK_RANGE(cond) CHECK_S(!!(cond)) << " {STARML::kOutOfRange} "
#define STARML_CHECK_INTERNAL(cond) \
  CHECK_S(!!(cond)) << " {STARML::kInternalError} "
#define STARML_CHECK_MATH(cond) CHECK_S(!!(cond)) << " {STARML::kMathError} "
#define STARML_CHECK_SUPPORTED(cond) \
  CHECK_S(!!(cond)) << " {STARML::kNotSupported} "

#define STARML_VLOG_IS_ON(verbosity) \
  ((verbosity) <= loguru::current_verbosity_cutoff())
#endif  // STARML_LOG_PREFIX
