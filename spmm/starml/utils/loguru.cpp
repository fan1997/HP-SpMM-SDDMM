...
//
#undef STARML_USE_THROW_EXCEPTION
#define STARML_USE_THROW_EXCEPTION 0
//
//		This block should be deleted
// }

#if STARML_USE_THROW_EXCEPTION
#include "STARML/api_c/error_handle.h"
#endif
...
#if STARML_USE_THROW_EXCEPTION
static fatal_handler_t s_fatal_handler = &STARML::PrepareException;
#else
...
void set_thread_name(const char* name) {
  ...
  strncpy_s(get_thread_name_win32(), LOGURU_THREADNAME_WIDTH + 1, name,
            _TRUNCATE);
...
void get_thread_name(char* buffer, unsigned long long length,
  ...
  if (buffer[0] == 0) {
    ...
    if (right_align_hext_id) {
      snprintf(buffer, length, "%*X", static_cast<int>(length - 1),
...
std::string stacktrace_as_stdstring(int skip) {
  ...
  for (int i = num_frames - 1; i >= skip; --i) {
    char buf[1024];
    Dl_info info;
    if ((callstack[i], &info) && info.dli_sname) {
      char* demangled = NULL;
      int status = -1;
...
static void print_preamble_header(char* out_buff, size_t out_buff_size) {
  ...
  if (g_preamble_file && pos < out_buff_size) {
    pos += snprintf(out_buff + pos, out_buff_size - pos, "%*s:line  ",
...
static void print_preamble(char* out_buff, size_t out_buff_size,
  ...
  if (g_preamble_verbose && pos < out_buff_size) {
    pos += snprintf(out_buff + pos, out_buff_size - pos, "%4s", level_buff);
  }
  if (g_preamble_pipe && pos < out_buff_size) {
    pos += snprintf(out_buff + pos, out_buff_size - pos, "| ");
  }
  ...
  if (!stack.empty()) {
    ...
    for (auto entry : stack) {
      ...
      auto prefix = textprintf("[ErrorContext] {.{}s}:{:-5u} {:-20s} ",
                               filename(entry->_file), LOGURU_FILENAME_WIDTH,
